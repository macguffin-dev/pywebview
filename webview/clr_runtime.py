...
#: Assemblies directly in ``webview/lib`` target .NETFramework; the
#: .NETCoreApp variants live in this subdirectory.
...
def _load(runtime: str) -> None:
    ...
    environment. :func:`select_runtime` has already set it; setting it again
...
def select_runtime(load: Callable[[str], None] | None = None) -> str:
    ...
    :func:`is_coreclr` — and therefore assembly resolution — stays truthful.
    """
    load = load or _load
